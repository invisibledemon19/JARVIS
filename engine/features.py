...
def hotword():
    """Listen for hotword using keyboard shortcut (Win+J) instead of Porcupine."""
    print("Hotword listener active — press Win+J to activate JARVIS")
    try:
        while True:
            keyboard.wait('win+j')
            print("hotword detected")
            # Small delay to prevent key repeat
            time.sleep(1)
    except Exception as e:
...
# chat bot (disabled — requires valid HugChat cookies)
def chatBot(query):
    speak("Chatbot is not configured yet. Please use Gemini search instead.")
